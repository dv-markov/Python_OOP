...
class Function:
    ...
    @staticmethod
    def __verify_numbers(num, tp):
        if type(num) not in (int, float):
            raise TypeError(f'{tp} должно быть числом')

    def __add__(self, other):
        self.__verify_numbers(other, 'смещение')
        obj = self.__class__(self)
        obj._bias = self._bias + other
        return obj

    def __mul__(self, other):
        self.__verify_numbers(other, 'амплитуда')
        obj = self.__class__(self)
...
class Linear(Function):
    def __init__(self, *args):
        ...
        if len(args) == 2:
            ...
            self._k, self._b = args[0].get_kb()
        else:
            raise AttributeError('Неверный ввод аргументов при создании класса Liner')
